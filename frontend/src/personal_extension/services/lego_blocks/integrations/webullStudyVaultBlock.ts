...
// Two source patterns:
//   <executionRoot>/watchlist/*.md                — watchlist (not-held) records
...
export async function scanWebullStudyVaultBlock(executionRoot: string): Promise<WebullStudyVaultScanBlock> {
  ...
  // 1) Watchlist (not-held) records: <root>/watchlist/*.md
  if (rootEntries.folders.includes('watchlist')) {
    const watchlistEntries = await listFolderEntries(`${trimmedRoot}/watchlist`)
  ...
  for (const folder of rootEntries.folders) {
    ...
    const expectedFile = `${folder.toLowerCase()}-study.md`
    const tickerEntries = await listFolderEntries(`${trimmedRoot}/${folder}`)
    if (!tickerEntries.files.includes(expectedFile)) continue
    const loaded = await readRecordBlock(trimmedRoot, `${folder}/${expectedFile}`, 'held-folder', warnings)
    if (loaded) out.push(loaded)
  }
