...
export function buildMindmapPreviewFromContentOrch(params: {
  inputPath: string
  content: string
  options: MindmapBuildOptions
}): MindmapPreviewData {
  const started = nowMs()
  const buildStarted = nowMs()
  const built = buildMindmapSceneFromMarkdownBlock(params.content, params.inputPath, params.options)
  const buildMs = nowMs() - buildStarted
  const serializeStarted = nowMs()
  const sceneMarkdown = serializeMindmapSceneToMarkdownBlock(built.scene)
  const serializeMs = nowMs() - serializeStarted
...
export async function saveMindmapSceneFromContentOrch(params: {
  inputPath: string
  content: string
  options: MindmapBuildOptions
  outputPath?: string
  ...
  const started = nowMs()
  const outputPath = params.outputPath?.trim() || suggestMindmapOutputPathOrch(params.inputPath)
  const preview = buildMindmapPreviewFromContentOrch({
    inputPath: params.inputPath,
